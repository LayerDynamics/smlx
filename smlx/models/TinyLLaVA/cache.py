...
"""
KV cache implementations for TinyLLaVA.

This module provides enhanced KV cache with memory monitoring, quantization,
and automatic pressure relief for the TinyLlama language model used in TinyLLaVA.

Quick Start:
    >>> from smlx.models.TinyLLaVA import load
    >>> from smlx.models.TinyLLaVA.cache import make_cache
    >>>
...
def make_cache(
    ...
    """
    Create KV cache for TinyLLaVA's TinyLlama language model with enhanced features.
    ...
    Args:
        language_model: TinyLlama language model
    ...
    if cache_type == "auto":
        ...
        if max_kv_size is None:
            ...
            # Get model config
            config = language_model.layers[0].self_attn

            limit_mgr = CacheLimitManager(
                model_size_gb=model_size_gb,
                target_memory_gb=target_memory_gb,
            )
            # TinyLlama config: 22 layers, 32 heads, head_dim=64, 4 kv_heads (GQA)
        ...
        manager = KVCacheManager.create_rotating(
            ...
            keep=0,  # TinyLlama uses standard RoPE, no special keep tokens
            enable_monitoring=enable_monitoring,
        )
...
def make_cache_with_monitoring(
    ...
    """
    num_layers = len(language_model.layers)
    model_size_gb = 1.1  # TinyLlama 1.1B
