...
class Idefics3Connector(nn.Module):
    ...
    Args:
        config: ModelConfig with scale_factor and hidden sizes (optional if individual params provided)
        vision_hidden_size: Vision encoder hidden size (optional if config provided)
        text_hidden_size: Text model hidden size (optional if config provided)
        scale_factor: Pixel shuffle scale factor (optional if config provided)
    ...
    def __init__(
        ...
        if config is not None:
            ...
            if vision_hidden_size is None or text_hidden_size is None or scale_factor is None:
                raise ValueError(
                    "Either config or (vision_hidden_size, text_hidden_size, scale_factor) must be provided"
                )
            self.scale_factor = scale_factor
            input_size = vision_hidden_size * (scale_factor**2)
            output_size = text_hidden_size

        self.modality_projection = MLP(input_size, output_size)
