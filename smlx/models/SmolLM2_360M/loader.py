...
"""
Model loading utilities for SmolLM2-360M-Instruct.

Handles downloading from HuggingFace Hub, loading weights, and initializing tokenizers.
Uses shared utilities from smlx.utils.
"""
...
def load_model_from_path(
    ...
    Example:
        >>> model, config = load_model_from_path("./models/smollm2-360m")
    """
    ...
    weights = load_weights(model_path, lazy=lazy)
...
def load(
    model_path: Union[str, Path] = "mlx-community/SmolLM2-360M-Instruct",
    lazy: bool = False,
    quantize: Optional[QuantizePreset] = None,
    quantization_config: Optional[dict] = None,
):
    """
    Load SmolLM2-360M model and tokenizer with optional quantization.
    ...
    Args:
        model_path: Either:
            ...
            - HuggingFace model ID (e.g., "mlx-community/SmolLM2-360M-Instruct")
        lazy: If True, use lazy loading for weights
        quantize: Quantization preset:
    ...
    tokenizer_obj = load_tokenizer(local_path)
...
def save_model(
    ...
    save_weights(weights, save_path / "model.safetensors")
