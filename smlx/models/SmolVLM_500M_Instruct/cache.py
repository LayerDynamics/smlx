...
"""
KV cache implementations for SmolVLM-500M-Instruct.

This module provides enhanced KV cache with memory monitoring, quantization,
and automatic pressure relief for the SmolLM2-360M language model used in SmolVLM-500M.

Quick Start:
    >>> from smlx.models.SmolVLM_500M_Instruct import load
    >>> from smlx.models.SmolVLM_500M_Instruct.cache import make_cache
    >>>
...
def make_cache(
    ...
    """
    Create KV cache for SmolVLM-500M's SmolLM2-360M language model with enhanced features.
    ...
    Args:
        language_model: SmolLM2-360M language model
    ...
    if (
        ...
        # SmolLM2-360M uses NoPE with keep=4 pattern
        return utils_make_cache(num_layers, max_kv_size=max_kv_size, keep=4)

    # New mode: use enhanced KVCacheManager
    # SmolLM2-360M model is ~1.0GB (360M params * 2 bytes/param for fp16 ≈ 720MB, round to 1.0GB)
    model_size_gb = 1.0
    ...
    if cache_type == "auto":
        ...
        if max_kv_size is None:
            ...
            # SmolLM2-360M config: 32 layers, 15 heads, head_dim=64, 5 kv_heads (GQA)
            # head_dim = 960 / 15 = 64
...
def make_cache_with_monitoring(
    ...
    """
    num_layers = language_model.num_hidden_layers
    model_size_gb = 1.0  # SmolLM2-360M is ~1.0GB
